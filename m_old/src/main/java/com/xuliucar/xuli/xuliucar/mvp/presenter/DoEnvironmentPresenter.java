...
import com.xuliucar.xuli.xuliucar.mvp.model.DoEnvironmentModel;
...
public class DoEnvironmentPresenter extends BasePresenterImpl implements CallBackListener<EnvironmentBean> {
    private DoEnvironmentModel mModel;
    private EnvironmentView mView;
    private Gson mGson = new Gson();
    private Handler mHandler;

    public DoEnvironmentPresenter(EnvironmentView view) {
        mView = view;
        mModel = new DoEnvironmentModel(this);
    }
    public void getData(){
        if(!NetWorkUtil.isNetWorkAvailable(App.getContext())){
            mView.showError();
        }else {
            mModel.getDoEnvironment();
        }
    }

    @Override
    public void OnSuccess(EnvironmentBean bean) {
        if(bean.isSuccess()){
            App.mCache.put(CacheName.DOENVIRONMENT,mGson.toJson(bean));
    ...
    public void loadMore(int start, int end) {
        ...
        EnvironmentBean bean = mGson.fromJson(App.mCache.getAsJSONObject(CacheName.DOENVIRONMENT).toString(),EnvironmentBean.class);
        int size = bean.getData().getInfo().size();
