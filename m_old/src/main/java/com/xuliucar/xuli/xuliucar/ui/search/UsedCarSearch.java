...
import com.xuliucar.xuli.xuliucar.adapter.UsedCarAdapter;
...
public class UsedCarSearch extends BaseActivity implements SearchView.SearchViewListener{
    private UsedCarAdapter resultAdapter;
    ...
    private void getData(){
        ...
        new Handler().postDelayed(new Runnable() {
            @Override
            public void run() {
             StockCarBean bean = mGson.fromJson(App.mCache.getAsJSONObject(CacheName.USEDCAR).toString(),StockCarBean.class);
    ...
    private void getAutoCompleteData(String text) {
        ...
        if (autoCompleteAdapter == null) {
            autoCompleteAdapter = new SearchAdapter(UsedCarSearch.this,autoCompleteData);
        } else {
    ...
    private void getResultData(String text) {
        if (resultData == null) {
            ...
            for (int i = 0; i < stockCarBean.size(); i++) {
                if (stockCarBean.get(i).getPlates().contains(text.trim())) {
                    resultData.add(stockCarBean.get(i));
                }
            }
        }
        if (resultAdapter == null) {
            resultAdapter = new UsedCarAdapter(this, resultData);
        } else {
