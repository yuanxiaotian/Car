...
import com.xuliucar.xuli.xuliucar.mvp.model.PassInsuranceModel;
...
public class PassInsurancePresenter extends BasePresenterImpl implements CallBackListener<InSuranceBean> {
    private PassInsuranceModel mModel;
    private InsuranceView mView;
    private Handler mHandler;
    private Gson mGson = new Gson();

    public PassInsurancePresenter(InsuranceView view) {
        mView = view;
        mModel = new PassInsuranceModel(this);
    }
    public void getData(){
        if(!NetWorkUtil.isNetWorkAvailable(App.getContext())){
            mView.showError();
        }else {
            mModel.getPassInsurance();
        }
    }
    ...
    public void OnSuccess(InSuranceBean bean) {
        if(bean.isSuccess()){
            App.mCache.put(CacheName.PASSINSURANCE,mGson.toJson(bean));
    ...
    public void loadMore(int start, int end) {
        ...
        InSuranceBean bean = mGson.fromJson(App.mCache.getAsJSONObject(CacheName.PASSINSURANCE).toString(),InSuranceBean.class);
        int size = bean.getData().getInfo().size();
