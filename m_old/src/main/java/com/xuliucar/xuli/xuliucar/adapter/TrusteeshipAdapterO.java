 ...
 *
 */
public class TrusteeshipAdapterO extends BaseAdapter {
    private final Context context;
    private Map<Integer, Boolean> map;
    private OnSelectedItemChanged listener;
    private int isdisplay;
    private List<TrusteeshipBean.DataBean.InfoBean> commentBeanList;
    private Dialog dialog;
    private List<String> phoeList;
    private List<String> idList;

    public TrusteeshipAdapterO(Context context,List<TrusteeshipBean.DataBean.InfoBean> commentBeanList){
    ...
    public TrusteeshipAdapterO(Context context, List<TrusteeshipBean.DataBean.InfoBean> commentBeanList, int isdisplay){
        this.context = context;
        this.commentBeanList = commentBeanList;
    ...
    public TrusteeshipAdapterO(Context context, List<TrusteeshipBean.DataBean.InfoBean> mFullLoadList, int isdisplay, OnSelectedItemChanged listener){
        this.context = context;
        this.commentBeanList = mFullLoadList;
        this.isdisplay = isdisplay;
        map = new HashMap<>();
        for (int i = 0; i < mFullLoadList.size(); i++) {
            map.put(i, false);
        }
    ...
    public View getView(final int i, View view, ViewGroup viewGroup) {
        ...
        if(isdisplay == 0){
            final TrusteeshipBean.DataBean.InfoBean commentBean = commentBeanList.get(i);
            ((TextView)holder.getView(R.id.odt_plates)).setText(commentBean.getPlates());
            ((TextView)holder.getView(R.id.odt_ownername)).setText(commentBean.getOwnername());
            ((TextView)holder.getView(R.id.odt_trstime)).setText(commentBean.getTrstime());
            ((TextView)holder.getView(R.id.odt_tretime)).setText(commentBean.getTretime());
           final String rphone = commentBean.getOwnerphone();
            final String rphone2 = commentBean.getOwnerphone2();
            ...
            season_msg_layout.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    if(rphone.equals("") && rphone2.equals("")){
                        ...
                        PhoneMessage.selectSendStyle(context, idList, "车主", "过期", StringConfig.OD_INTRUST + StringConfig.COMPANYNAME, StringConfig.TYPE_INTRUST, rphone, rphone2);
                    }
                }
            });
            ...
            final TrusteeshipBean.DataBean.InfoBean fullLoad = commentBeanList.get(i);
            ((TextView)holder.getView(R.id.odt_plates)).setText(fullLoad.getPlates());
            ((TextView)holder.getView(R.id.odt_ownername)).setText(fullLoad.getOwnername());
            ((TextView)holder.getView(R.id.odt_trstime)).setText(fullLoad.getTrstime());
            ((TextView)holder.getView(R.id.odt_tretime)).setText(fullLoad.getTretime());
            ...
            season_msg_layout.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    PhoneMessage.selectSendStyle(context, idList, "车主", "过期", StringConfig.OD_INTRUST + StringConfig.COMPANYNAME, StringConfig.TYPE_INTRUST, phone, phone2);
                }
            });
    ...
    public void selectAll() { // 全选
        ...
        for (int i = 0; i < commentBeanList.size(); i++) {
            TrusteeshipBean.DataBean.InfoBean fullLoad = commentBeanList.get(i);
            map.put(i, true);
            PreferencesUtils.putSharePre(context, "GroupMsg", "phone" + i, fullLoad.getOwnerphone());
