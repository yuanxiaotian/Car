 ...
 */
public class InsuranceInfoma extends BaseFragment implements View.OnClickListener{

    private ImageView compulsory_insurance_p_img,Commercial_insurance_p_img;
    ...
    private RelativeLayout compulsory_insurance_p;
    private RelativeLayout commercial_insurance_p;
    private static final int REQUEST_CAMERA_PERMISSION = 0x01;

    @Override
    protected void setContentView() {
        setContentView(R.layout.insurance_infoma);
    }

    @Override
    protected void initView() {
        compulsory_insurance_p = getViewById(R.id.compulsory_insurance_p);
        commercial_insurance_p = getViewById(R.id.Commercial_insurance_p);
        compulsory_insurance_p_img =getViewById(R.id.compulsory_insurance_p_img);
        Commercial_insurance_p_img = getViewById(R.id.Commercial_insurance_p_img);
    ...
    protected void setListener() {
        compulsory_insurance_p.setOnClickListener(this);
        commercial_insurance_p.setOnClickListener(this);
    }
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if(data != null){
            if(requestCode == 101 && resultCode == 1001){
                ShowImageDialog.getInstance().ImageWithRound(compulsory_insurance_p_img,data,dialog);
            }else if(requestCode == 102 && resultCode == 1001){
                ShowImageDialog.getInstance().ImageWithRound(Commercial_insurance_p_img,data,dialog);
    ...
    public void onClick(View views) {
        int i = views.getId();
        if (i == R.id.compulsory_insurance_p) {
            if (App.from.equals("")) {
                ToastUtil.showShortToast(getActivity(), "没有填写表单号");
            } else {
                showOriginalPic(UrlConfig.is1_url, 0);
                tag = 101;
            }

        } else if (i == R.id.Commercial_insurance_p) {
            if (App.from.equals("")) {
                ToastUtil.showShortToast(getActivity(), "没有填写表单号");
            } else {
                showOriginalPic(UrlConfig.is2_url, 0);
                tag = 102;
            }
