...
public class NewCarPresenter extends BasePresenterImpl implements CallBackListener<StockCarBean> {
    private StockCarModel mModel;
    private StockCarView mView;

    public NewCarPresenter(StockCarView view) {
        mView = view;
    ...
    public void OnSuccess(StockCarBean bean) {
        if (bean.isSuccess()) {
            ...
            for (int i = 0; i < info.size(); i++) {
                ...
                if (!TextUtils.isEmpty(info.get(i).getPlates()) && (length > 2)) {
                    info.remove(i);
                    i--;
                }
            }
