...
import com.xuliucar.xuli.xuliucar.mvp.model.PassYearRoadTModel;
...
public class PassYearRoadTPresenter extends BasePresenterImpl implements CallBackListener<YearRoadTBean> {
    private PassYearRoadTModel mModel;
    private YearRoadTView mView;
    private Handler mHandler;
    private Gson mGson = new Gson();

    public PassYearRoadTPresenter(YearRoadTView view) {
        mView = view;
        mModel = new PassYearRoadTModel(this);
    }
    public void getData(){
        if(!NetWorkUtil.isNetWorkAvailable(App.getContext())){
            mView.showError();
        }else {
            mModel.getPassYearRoadT();
        }
    }
    ...
    public void OnSuccess(YearRoadTBean bean) {
        if(bean.isSuccess()){
            App.mCache.put(CacheName.PASSYEARROADT,mGson.toJson(bean));
