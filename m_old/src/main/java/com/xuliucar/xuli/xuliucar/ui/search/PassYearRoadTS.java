...
public class PassYearRoadTS extends BaseActivity implements SearchView.SearchViewListener{
    private YearRoadTAdapter resultAdapter;
    private ListView lvResults;
    private List<YearRoadTBean.DataBean.InfoBean> roadeBeanList;
    private List<YearRoadTBean.DataBean.InfoBean> resultData;
    ...
    private void getData(){
        roadeBeanList = new ArrayList<>();
        allData = new ArrayList<>();
        new Handler().postDelayed(new Runnable() {
            @Override
            public void run() {
                YearRoadTBean bean = mGson.fromJson(App.mCache.getAsJSONObject(CacheName.PASSYEARROADT).toString(), YearRoadTBean.class);
                int size = bean.getData().getInfo().size();
                roadeBeanList.addAll(bean.getData().getInfo());
    ...
    private void getAutoCompleteData(String text) {
        ...
        if (autoCompleteAdapter == null) {
            autoCompleteAdapter = new SearchAdapter(PassYearRoadTS.this,autoCompleteData);
        } else {
    ...
    private void getResultData(String text) {
        if (resultData == null) {
            ...
            for (int i = 0; i < roadeBeanList.size(); i++) {
                if (roadeBeanList.get(i).getPlates().contains(text.trim())) {
                    resultData.add(roadeBeanList.get(i));
                }
            }
        }
        if (resultAdapter == null) {
            resultAdapter = new YearRoadTAdapter(this, resultData);
        } else {
