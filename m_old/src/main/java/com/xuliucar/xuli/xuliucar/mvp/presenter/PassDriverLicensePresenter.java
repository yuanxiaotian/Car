...
import com.xuliucar.xuli.xuliucar.mvp.model.PassDriverLicenseModel;
...
public class PassDriverLicensePresenter extends BasePresenterImpl implements CallBackListener<DriverLicenseBean> {
    private PassDriverLicenseModel mModel;
    private DriverLicenseView mView;
    private Handler mHandler;
    private Gson mGson = new Gson();


    public PassDriverLicensePresenter(DriverLicenseView view) {
        mView = view;
        mModel = new PassDriverLicenseModel(this);
    }
    public void getData(){
        if(!NetWorkUtil.isNetWorkAvailable(App.getContext())){
            mView.showError();
        }else {
            mModel.getDriverLicense();
        }
    }
    ...
    public void OnSuccess(DriverLicenseBean bean) {
        if(bean.isSuccess()){
            App.mCache.put(CacheName.PASSDRIVERLICENSE,mGson.toJson(bean));
