...
import com.xuliucar.xuli.xuliucar.adapter.EnvironmentAdapterO
import com.xuliucar.xuli.xuliucar.base.BaseActivity
import com.xuliucar.xuli.xuliucar.bean.EnvironmentBean
import com.xuliucar.xuli.xuliucar.mvp.presenter.PassEnvironmentPresenter
import com.xuliucar.xuli.xuliucar.mvp.view.EnvironmentView
import com.xuliucar.xuli.xuliucar.ui.homePage.allCar.CarDetail
import com.xuliucar.xuli.xuliucar.ui.search.PassEnvironmentS
import com.xuliucar.xuli.xuliucar.utils.Pull
import com.xuliucar.xuli.xuliucar.utils.ToastUtil
import java.util.*

class PassEnvironment : BaseActivity(), AdapterView.OnItemClickListener, View.OnClickListener, EnvironmentView {
    ...
    private var commentBeanList: MutableList<EnvironmentBean.DataBean.InfoBean>? = null
    private var adapter: EnvironmentAdapterO? = null
    private var starts = 10
    private var ends = 10
    private val size = 10
    private var msg: String? = "1"
    private var mPresenter: PassEnvironmentPresenter? = null
    ...
    override fun initContentView(savedInstanceState: Bundle) {
        ...
        mPresenter = PassEnvironmentPresenter(this)
    }
...
//        matter_refreshLayout!!.beginRefreshing()
    ...
    override fun setListener() {
        ...
        mTo_search!!.setOnClickListener {
            val intent = Intent(this@PassEnvironment, PassEnvironmentS::class.java)
    ...
    private fun initData(index: Int) {
        when (index) {
            0 -> {
                adapter = EnvironmentAdapterO(this@PassEnvironment, commentBeanList, 0)
                matter_listView!!.adapter = adapter
            }
            1 -> {
                adapter = EnvironmentAdapterO(this@PassEnvironment, commentBeanList!!, 1, EnvironmentAdapterO.OnSelectedItemChanged { })
                matter_listView!!.adapter = adapter
            }
        }
    }
    ...
    override fun onItemClick(adapterView: AdapterView<*>, view: View, i: Int, l: Long) {
        ...
        val bundle = Bundle()
        bundle.putSerializable("enviromentData", commentBean)
...
//            } else {
//                PhoneMessage.selectSendStyle(this@PassEnvironment, phoeList, idList, "车主", "过期", StringConfig.OD_EPM + StringConfig.COMPANYNAME, StringConfig.TYPE_EPM)
//            }
//
//        }
    }

    override fun startLoad(been: MutableList<EnvironmentBean.DataBean.InfoBean>, size: Int) {
        this.length = size
//        Handler().postDelayed({
...
//        }, 200)
    ...
    override fun loadMore(been: MutableList<EnvironmentBean.DataBean.InfoBean>, size: Int) {
//        Handler().postDelayed({
//            commentBeanList!!.addAll(been)
//            adapter!!.notifyDataSetChanged()
...
//        }, 200)
    ...
    override fun loadAll(been: MutableList<EnvironmentBean.DataBean.InfoBean>) {
        commentBeanList!!.addAll(been)
        adapter!!.notifyDataSetChanged()
    }
