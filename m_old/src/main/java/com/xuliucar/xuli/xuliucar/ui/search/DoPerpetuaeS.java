...
import com.xuliucar.xuli.xuliucar.adapter.PerpetuaeAdapter;
...
public class DoPerpetuaeS extends BaseActivity implements SearchView.SearchViewListener,AdapterView.OnItemClickListener{
    private PerpetuaeAdapter resultAdapter;
    private ListView lvResults;
    private List<DoPerpetuaeBean.DataBean.InfoBean> mInfoBeanList;
    private List<DoPerpetuaeBean.DataBean.InfoBean> resultData;
    ...
    private void getData() {
        ...
        new Handler().postDelayed(new Runnable() {
            @Override
            public void run() {
                DoPerpetuaeBean bean = mGson.fromJson(App.mCache.getAsJSONObject(CacheName.DOPERPETUAE).toString(),DoPerpetuaeBean.class);
    ...
    private void getAutoCompleteData(String text) {
        ...
        if (autoCompleteAdapter == null) {
            autoCompleteAdapter = new SearchAdapter(DoPerpetuaeS.this,autoCompleteData);
        } else {
    ...
    private void getResultData(String text) {
        if (resultData == null) {
            ...
            for (int i = 0; i < mInfoBeanList.size(); i++) {
                String search = mInfoBeanList.get(i).getPlates()+" "+mInfoBeanList.get(i).getOwner();
        ...
        if (resultAdapter == null) {
            resultAdapter = new PerpetuaeAdapter(this, resultData);
        } else {
    ...
    public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
        DoPerpetuaeBean.DataBean.InfoBean infoBean = resultData.get(i);
        Intent intent = new Intent(view.getContext(), CarDetail.class);
        Bundle bundle = new Bundle();
        bundle.putSerializable("doperpetuaeData", infoBean);
        bundle.putString("page","0");
        bundle.putString("pageName","doperpetuae");
