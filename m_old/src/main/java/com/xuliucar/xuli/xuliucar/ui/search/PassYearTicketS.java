...
public class PassYearTicketS extends BaseActivity implements SearchView.SearchViewListener,AdapterView.OnItemClickListener{
    ...
    private void getAutoCompleteData(String text) {
        ...
        if (autoCompleteAdapter == null) {
            autoCompleteAdapter = new SearchAdapter(PassYearTicketS.this,autoCompleteData);
        } else {
    ...
    private void getResultData(String text) {
        if (resultData == null) {
            ...
            for (int i = 0; i < commentBeanList.size(); i++) {
                if (commentBeanList.get(i).getPlates().contains(text.trim())) {
    ...
    public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
        YearTicketBean.DataBean.InfoBean commonBean = resultData.get(i);
