...
public class DoCompanyDocS extends BaseActivity implements SearchView.SearchViewListener{
    private DoCompanyAdapter resultAdapter;
    private ListView lvResults;
    private List<DoCompanyDocBean.DataBean.InfoBean> commentBeanList;
    private List<DoCompanyDocBean.DataBean.InfoBean> resultData;
        ...
        new Handler().postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                for (int i = 0; i < size; i++) {
                    allData.add(bean.getData().getInfo().get(i).getLicname());
                }
    ...
    private void getAutoCompleteData(String text) {
        ...
        if (autoCompleteAdapter == null) {
            autoCompleteAdapter = new SearchAdapter(DoCompanyDocS.this,autoCompleteData);
        } else {
    ...
    private void getResultData(String text) {
        if (resultData == null) {
            ...
            for (int i = 0; i < commentBeanList.size(); i++) {
                if (commentBeanList.get(i).getLicname().contains(text.trim())) {
                    resultData.add(commentBeanList.get(i));
                }
            }
        }
        if (resultAdapter == null) {
            resultAdapter = new DoCompanyAdapter(this, resultData);
        } else {
